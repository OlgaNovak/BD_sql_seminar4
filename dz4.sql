...
SELECT COUNT(*) AS 'Количетво лайков' FROM likes l
RIGHT JOIN profiles p ON l.user_id=p.user_id
WHERE birthday>'2011-06-05';

-- Задача 2. Определить кто больше поставил лайков (всего): мужчины или женщины.

SELECT 
    IF ((SELECT COUNT(*) AS 'Лайки, поставленные мужчинами' FROM likes l
         RIGHT JOIN profiles p ON l.user_id=p.user_id
		 WHERE gender='m')>(SELECT COUNT(*) AS 'Лайки, поставленные женщинами' FROM likes l
         RIGHT JOIN profiles p ON l.user_id=p.user_id
         WHERE gender='f'),'Мужчины','Женщины')
AS 'Кто больше поставил лайков:';

-- Задача 3. Вывести всех пользователей, которые не отправляли сообщения.

SELECT COUNT(*) AS 'Пользователи, которые не отправляли сообщения' FROM users u
LEFT JOIN messages m ON u.id=m.from_user_id
